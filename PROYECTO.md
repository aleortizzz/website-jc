...
- **404 de URLs legado (2026-09-04)**: Google todavía tiene indexadas URLs del sitio viejo `jcherrajes.com` (ej. `/barandasvarias.html`, `/barandasvidrotemplado.html`) que ya no existen. Como el redirect de dominio canónico preserva el path, esas URLs terminaban en `jcbarandas.com.ar/lo-que-sea` con un 404 feo de IIS. Fix: regla `web.config` al final de las rules — cualquier request que no sea a un archivo o carpeta real, 301 a `/`. Se resuelve solo con el tiempo a medida que Google recrawlea y actualiza el índice.
- **Bug de caché encontrado (2026-09-03)**: el `clientCache` de `<staticContent>` (7 días) se aplica a TODO archivo estático que sirve IIS, `.html` incluido — el cliente veía versiones viejas del sitio porque el navegador cacheaba las páginas por una semana. Fix: regla `outboundRules` en `web.config` que sobreescribe el header `Cache-Control` a `no-cache, no-store, must-revalidate` solo para `.html`/`.inc` (usa `RESPONSE_Cache_Control` como server variable — nombre de header con `-` pasa a `_` en IIS). Las imágenes/CSS/JS siguen con el caché largo, que ahí sí conviene. Recordar volver a poner caché larga en `.html` una vez que el sitio deje de cambiar seguido (ver Pendientes).
